...
def bin_spikes(spike_times, dt, wdw_start, wdw_end):
    ...
    spike_times: an array of arrays
        an array of neurons. within each neuronw's array is an array containing all the spike times of that neuron
...
def bin_output(outputs, output_times, dt, wdw_start, wdw_end, downsample_factor=1):
    ...
    if downsample_factor != 1:  # Don't downsample if downsample_factor=1
        downsample_idxs = np.arange(0, output_times.shape[0],
                                    downsample_factor)  # Get the idxs of values we are going to include after downsampling
    ...
    for i in trange(num_bins):  # Loop through bins
        idxs = np.where((np.squeeze(output_times) >= edges[i]) & (np.squeeze(output_times) < edges[i + 1]))[
            0]  # Indices to consider the output signal (when it's in the correct time range)
