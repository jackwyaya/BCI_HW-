...
class KalmanFilterRegression(object):
    ...
    def fit(self,X_kf_train,y_train):
        ...
        X2 = X[:,1:]#预测
        X1 = X[:,0:nt-1]#现在
        A=X2*X1.T*inv(X1*X1.T) #Transition matrix 最小二乘的解析解
        ...
        '''
        X2:真实，A*X1:预测 协方差
        W 是卡尔曼滤波中的过程噪声协方差矩阵，用来表示状态变量在相邻时间步骤之间的真实变化与状态方程模型之间的不确定性。
        '''
        ...
        # Q = np.random.rand(Q.shape[0],Q.shape[1]) #Randomize Q to prevent singular matrix errors
        params=[A,W,H,Q]
        self.model=params
    ...
    def predict(self,X_kf_test,y_test):
        ...
        #Extract parameters
        A,W,H,Q=self.model
        # W = np.random.rand(W.shape[0],W.shape[1]) #Randomize W to prevent singular matrix errors
        ...
        # P,P_m = np.random.rand(P.shape[0],P.shape[1]),np.random.rand(P_m.shape[0],P_m.shape[1]) #Randomize P and P_m to prevent singular matrix errors
        state=X[:,0] #Initial state
        states[:,0]=np.copy(np.squeeze(state))

        #Get predicted state for every time bin
        for t in trange(X.shape[1]-1):
            #Do first part of state update - based on transition matrix
            P_m=A*P*A.T+W #协方差矩阵转移到下一时刻
            state_m=A*state
